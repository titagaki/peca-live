...
代わりに **GET エンドポイントを外部スケジューラ（Heroku Scheduler 等）から定期的に叩く** 設計になっている。
...
`cache_store` は未設定（Rails デフォルト）。Heroku の本番ではファイルストア (`tmp/cache`) で、dyno 再起動で消える・dyno 間で共有されない。

| キー | TTL | 内容 | 使用箇所 |
| --- | --- | --- | --- |
| `Api::V1::ChannelsController/get_channels` | 1 分 | `updateYPChannels` の生の結果 | `record_history`, `fetch_channels` |
...
| `api/v1/peercast/show` | 1 分 | `getStatus` の抜粋 | `/api/v1/peercast` |
